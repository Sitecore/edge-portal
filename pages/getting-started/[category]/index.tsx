          ...
          <Grid container>
            ...
            <Grid item xs={12} md={9}>
              ...
              <Typography variant="body2" gutterBottom>
                <div
                  dangerouslySetInnerHTML={{ __html: currentCategory.Body }}
                />
              </Typography>
            </Grid>
          </Grid>
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  var currentCategory: Category = await GetCategory(
    "Getting Started",
    category
  );
...
export async function getStaticPaths(categories: Categories) {
  ...
  return {
    paths:
      categories.results?.map(({ Name }) => `/getting-started/${Name}`) ?? [],
    fallback: true,
  };
}
